...
| `tunnel_security` | server/client | 额外安全层。`reality` 用于包装 raw client/server 隧道；Vision flow 是 VLESS 专用语义。普通 TLS 不写在这里。 |
...
| --- | --- | --- | --- | --- | --- | --- |
| native | 支持 | 支持 | 支持 | 支持 | 仅 REALITY | tcptun-to-tcptun |
| vless | 支持 | 支持 | 支持 | 支持 | REALITY/Vision | VLESS TCP/UDP，REALITY/Vision，mux.cool |
| vmess | 支持 | 支持 | 支持 | 支持 | 仅 REALITY | VMess AEAD TCP/UDP，security none，mux.cool |
| trojan | 支持 | 支持 | tcptun-to-tcptun | 推荐 | 仅 REALITY | Trojan TCP/UDP |

## 配置建议

- 自己控制 client/server 两端，并希望性能和功能优先：优先使用 `native`；它也可以运行在 raw REALITY 外层之上。
