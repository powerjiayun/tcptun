...
For tcptun-to-tcptun deployments, `--tunnel-security reality` can wrap any tunnel protocol over raw transport. Vision flow is VLESS-specific, so set `--flow xtls-rprx-vision` only with `--tunnel-protocol vless`.
