...
func buildApp() *cmd.App {
	...
	app.Root = &cmd.Command{
		...
		Short:     "forward local mixed proxy traffic through the gateway port",
		Long: "Starts a local TCP listener for mixed proxy clients and forwards each connection " +
			"through the default gateway port. Upstream protocol defaults to SOCKS5.",
		Examples: []string{
			"tcptun",
			"tcptun local",
		...
		SetFlags: func(f *cmd.FlagSet) {
			...
			f.DurationVar(&cfg.HeartbeatInterval, "heartbeat-interval", cfg.HeartbeatInterval, "TCP keepalive heartbeat interval", "")
			f.DurationVar(&cfg.ConnectionIdleTimeout, "connection-idle-timeout", cfg.ConnectionIdleTimeout, "idle timeout before recycling TCP proxy connections", "")
			f.DurationVar(&cfg.UDPSessionTimeout, "udp-session-timeout", cfg.UDPSessionTimeout, "idle timeout before recycling UDP sessions", "")
			f.DurationVar(&cfg.RetryInitialInterval, "retry-initial-interval", cfg.RetryInitialInterval, "initial delay for exponential retry backoff", "")
			f.DurationVar(&cfg.RetryMaxInterval, "retry-max-interval", cfg.RetryMaxInterval, "maximum delay for exponential retry backoff", "")
